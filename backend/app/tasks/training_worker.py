...
async def run_training(ctx: dict, job_id: str, csv_path: str) -> dict:
    ...
    def progress_callback(label: str, fitted: int, total: int):
        ...
        # Use synchronous Redis set via run_coroutine_threadsafe would be complex;
        # instead, store progress in a simple way using the blocking redis
        ctx["_progress"] = {"label": label, "fitted": fitted, "total": total, "pct": pct}
